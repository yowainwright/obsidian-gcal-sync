...
export const fetchTodayEvents = async (
  ...
  const eventPromises = calendarIds.map(async (calendarId) => {
    try {
      return await fetchEventsFromCalendar(client, calendarId, timezone, startOfDay, endOfDay);
    } catch (error) {
      console.warn(`[GCal Sync] Failed to fetch from calendar ${calendarId}:`, error);
      return [];
    }
  });
